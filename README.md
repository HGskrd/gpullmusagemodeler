...
DeepSeek V4.1 Flash uses a separate CED/CSA2 path: 8B active prefill plus a bounded 128-token decoder replay, 16B active decode, four shared global KV pools, and eight sparse indexers with a 16,384-position cap for later decoder indexers. The optimized runtime cache uses 288 bytes per main row, 68 per index row, and 528 per FP8 local row including scales. Its 890 B/token global slope excludes the bounded local cache. Global KV is conservatively replicated on each TP rank and PP stage; attention reads are charged for every consuming layer. Prefill uses a conservative final-context attention rectangle and the busiest pipeline stage. The fixed KV layout assumes optimized serving kernels, independent of selected weight precision; reference inference buffers can be larger.

The V4.1 native 510.286 GB checkpoint includes 196B Engram parameters, vision, and DSpark tensors. Engram stays GPU-resident at FP8 (202.758 GB including scales), charged to its owning layers 1 and 14 rather than evenly divided across pipeline stages; host offload, RDMA prefetch, vision-encoder latency, persistent SSD cache, and DSpark speedup require runtime calibration and are not simulated. Native active compute/traffic uses exact routed-expert geometry with remaining operators conservatively charged as BF16. Quality and verbosity retain a low-confidence V4 Flash family proxy pending comparable evidence. Sources captured 2026-09-10: [official report](https://huggingface.co/deepseek-ai/DeepSeek-V4.1-Flash/blob/df42c109f1defefcbfcedbe7d905718a12266e40/DeepSeek_V41_Tech_Report.pdf) and config/index at the same revision.
